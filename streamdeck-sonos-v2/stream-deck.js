...
class StreamDeck {
    ...
    setImageURL(imageURL, texts, titleParameters, context) {
        ...
        return new Promise((resolve, reject) => {
            if(!imageURL) {
                console.warn('setImageURL called with empty imageURL');
            ...
            image.onerror = (error) => {
                ...
                this.logMessage(`Failed to load album art from ${imageURL}`);
                reject(error);
            };
            ...
            image.onload = () => {
                try {
                    ...
                    this.logMessage(`Canvas rendering error: ${error.message}`);
                    reject(error);
                }
            };
            ...
            const timeout = setTimeout(() => {
                console.warn('Image load timeout for URL:', imageURL);
                this.logMessage(`Album art load timeout for ${imageURL}`);
