...
function connectElgatoStreamDeckSocket(port, pluginUUID, registerEvent, info, actionInfo) {
    ...
    streamDeck.onConnected(() => {
        ...
        streamDeck.getGlobalSettings().then((globalSettings) => {
            ...
            const port = parseInt(globalSettings.port, 10) || 1400;
            sonos.connect(globalSettings.host, port, globalSettings.targetGroup || '');

            sonos.getAvailableGroups().then((groups) => {
                const select = document.getElementById('target-group');
                if (!select) {
                    return;
                }

                const selectedGroup = globalSettings.targetGroup || '';
                const options = groups
                    .filter((group) => group.coordinator && group.name)
                    .sort((a, b) => a.name.localeCompare(b.name));

                options.forEach((group) => {
                    const option = document.createElement('option');
                    option.value = group.coordinator;
                    option.selected = selectedGroup === group.coordinator;
